                       ...
                       do v <- ask
                          case v of
                            Right ar@(Json.Array a) ->
                                 do res <-
                                        local (const $ Right $ array $ drop h $ fromArray ar) ma
               ...
               Right (Json.Array ar) ->
                   case V.toList ar of
                     ...
                     _ ->
                         do res <-
                                forM (V.toList ar) $
                                     \el -> local (const $ Right el) (readSmart pFormat)
                            if null (lefts res)
                               then return $ Right $ rights res
                               else return $ Left []
               ...
               Right (Json.Array ar) ->
                   case V.toList ar of
                     ...
                     _ ->
                         do res <-
                                forM (V.toList ar)
                                     (\el -> local (const $ Right el) $ readSmart pFormatUnvers)
                            if null (lefts res)
                               then return $ Right $ rights res
                               else return $ Left []
               _ -> mismatch "Array" (show val)
